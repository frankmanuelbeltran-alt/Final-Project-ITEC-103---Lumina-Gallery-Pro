...
class ThumbnailLoader:
    ...
    def _process_queue(self):
        while not self._shutdown:
            try:
                # PRIORITY: immediate queue first
                try:
...
class LuminaGalleryProMax:
    def __init__(self, root):
        ...
        # FIX 1: Set fullscreen BEFORE anything else
        self.root.attributes('-fullscreen', True)
        self.root.update_idletasks()  # Force immediate application
        ...
        # Initialize fonts FIRST before any widget creation
        self._init_fonts()
        ...
        # FIX 2: Create widgets AFTER fullscreen is set
        self.create_widgets()
        self.bind_events()
    ...
    def _create_sidebar_button(self, parent, text, command):
        """Create a sidebar button with proper hover effects"""
    ...
    def update_scrollregion(self):
        """Update the scroll region based on content - FIXED for dynamic loading"""
        if not self.media:
            try:
                if self.grid_canvas.winfo_exists():
                    self.grid_canvas.config(scrollregion=(0, 0, 0, 0))
            except tk.TclError:
                pass
            return
        ...
        # Add padding at bottom for smoother scrolling
        height += 100
        
        try:
            if self.grid_canvas.winfo_exists():
                self.grid_canvas.config(scrollregion=(0, 0, 0, height))
                # Force update to ensure scrollbar reflects new region
    ...
    def _create_button(self, parent, text, command, is_accent=False, emoji="", 
        ...
        """
        Create a button that returns both the frame and the label widget.
        """
    ...
    def refresh_grid(self):
        ...
        try:
            ...
            # Update scroll region based on total items
    ...
    def toggle_fullscreen(self):
        ...
        try:
            if self.fullscreen:
                ...
                self.root.attributes('-fullscreen', True)
                # Force update for canvas to resize properly
                self.root.update_idletasks()
                # Re-render current image if in single view
                if self.view_mode == ViewMode.SINGLE and self.original_image:
                    self.root.after(100, self.reset_zoom)
            else:
                self.root.attributes('-fullscreen', False)
                # Restore previous geometry
                if hasattr(self, '_pre_fullscreen_geometry'):
                    self.root.geometry(self._pre_fullscreen_geometry)
                # Return to grid view when exiting fullscreen
                self.root.after(100, self.show_grid_view)
        except tk.TclError as e:
    ...
    def show_grid_view(self):
        ...
        if HAS_VLC and self.vlc_player:
            try:
                ...
                logger.debug(f"Video stop error: {e}")
        
        # Hide video UI elements safely
        try:
        ...
        # FIX: Safe frame switching with existence checks
        try:
            if hasattr(self, 'single_frame') and self.single_frame.winfo_exists():
                self.single_frame.pack_forget()
        except tk.TclError:
            pass
            
        try:
            if hasattr(self, 'slideshow_frame') and self.slideshow_frame.winfo_exists():
        ...
        # Update scroll region before refresh
        self.update_scroll_region()
        
        # Force thumbnail refresh with slight delay for frame to render
        self._clear_all_thumbnails()
        self.root.after(150, self.refresh_grid)
    ...
    def show_duplicates(self):
        ...
        self._clear_all_thumbnails()
        self.apply_filters()
        self.show_grid_view()  # ADD THIS LINE
...
"""
        ...
        if item.path not in self.loading_thumbs:
            ...
            self.thumb_loader.submit(
                ...
                lambda result, f=frame, i=idx, p=item.path:
                    self._on_thumbnail_ready(f, i, result, p)
            )
    ...
    def _load_thumbnail_image(self, path):
        """
        Load thumbnail image with proper error handling and caching.
        Returns tuple of (image, is_favorite, is_video, rating) or None on error.
        """
        try:
            item = self.media_by_path.get(path)
            if not item:
            ...
            if item.is_video:
                ...
                if cached:
                    return (cached, item.favorite, True, item.rating)
    ...
    def _apply_thumbnail_image(self, frame, idx, result):
        ...
        Handles widget destruction and stale data gracefully.
        """
        if not frame.winfo_exists():
            return
        ...
        # FIX 3: Store unique PhotoImage reference on frame to prevent garbage collection
    ...
    def _remove_thumbnail(self, idx):
        """Remove a thumbnail widget and cancel its loading task"""
        if idx in self.visible_thumbs:
            frame = self.visible_thumbs[idx]
            try:
                if frame.winfo_exists():
    ...
    def _clear_all_thumbnails(self):
        """Clear all thumbnail widgets and cancel loading tasks"""
    ...
    def get_visible_range(self):
        ...
        Returns (start, end) tuple of visible indices.
        """
    ...
    def _on_thumbnail_click(self, event, frame):
        ...
        Supports Ctrl+click for multi-select, Shift+click for range select.
        """
        if not frame.winfo_exists():
            return
    ...
    def smooth_scroll(self, event):
        """Handle smooth scrolling - FIXED with bounds checking"""
        if not hasattr(self, 'grid_canvas') or not self.grid_canvas.winfo_exists():
            return "break"
        ...
        if delta != 0:
            ...
            current_pos = self.grid_canvas.yview()
            scroll_top = current_pos[0]
            scroll_bottom = current_pos[1]
            
            # Check if we can scroll further
            if delta > 0 and scroll_bottom >= 0.99:
                # At bottom, don't scroll further
                pass
            elif delta < 0 and scroll_top <= 0.01:
                # At top, don't scroll further
                pass
            else:
...
├── config/
│   ├── __init__.py
│   └── settings.py            # Config, ViewMode, SortMode
├── core/
│   ├── __init__.py
│   ├── gallery_app.py         # Complete LuminaGalleryProMax class (~2000 lines)
...
│   ├── __init__.py
│   ├── tk_queue.py            # TkQueue
│   ├── thumbnail_loader.py    # ThumbnailLoader
│   └── background_worker.py   # BackgroundWorker
├── ui/
│   ├── __init__.py
│   ├── toast_manager.py       # ToastManager
│   └── ux_enhancement.py
└── utils/
    ├── __init__.py
    ├── logging_utils.py       # Logger
    ├── threading_utils.py     # ThreadSafeDict, ThreadSafeList
    └── exif_reader.py         # ExifReader'''
